...
- code will be implemented in `pygeoapi/validator/base.py` as an abstract base class with a `validate(data, partial) -> None` method
  - returns `None` on valid data payload
    - note that `partial` (default `False`) is added for HTTP PATCH workflow
...
Adopted on 2026-08-18 with +1 from justb4, kalxas, jorgejesus, francbartoli, tomkralidis
